% applying XY_VTE_DETECTION_F_TEST
...
%         VTE_LinearVelocity(startviewing:startviewing+windowsize,1))
    ...
    [VTE_Start,VTE_End] = XY_VTE_Detection_F_TEST(startviewing,startviewing+windowsize,VTE_PositionXY);
    ...
    for j = 1:length(VTE_Start)
        subplot(length(VTE_Start)+1,1,1)
        plot(VTE_PositionXY(VTE_Start(j):VTE_End(j),1),...
            VTE_PositionXY(VTE_Start(j):VTE_End(j),2),'Color','r')
